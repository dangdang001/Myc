...
coef=c(1,1,1,1,1,8) # coeffient of true model covariates: x1,x2,x3,x4,x5,myc


simu_myc_effect<-function(n,K,lambda,nb_mean,nb_size,coef_myc){
  ...
  fit_myc_gene <- coxph(Surv(time = df$surv, event = df$status) ~.-myc-surv-status-myc_score1-myc_score2-myc_score3_up-myc_score3_down, data = df)
  #summary(fit_myc_gene) 
  
  fit_myc_score1 <- coxph(Surv(time = df$surv, event = df$status) ~x1+x2+x3+x4+x5+myc_score1, data = df)
  #summary(fit_myc_score1) 
  
  fit_myc_score2 <- coxph(Surv(time = df$surv, event = df$status) ~x1+x2+x3+x4+x5+myc_score2, data = df)
  #summary(fit_myc_score2) 
  
  fit_myc_score3 <- coxph(Surv(time = df$surv, event = df$status) ~x1+x2+x3+x4+x5+myc_score3_up+myc_score3_down, data = df)
  #summary(fit_myc_score3) 
  
  # fit_myc_score4 <- coxph(Surv(time = df$surv, event = df$status) ~x1+x2+x3+x4+x5+myc_score4, data = df)
  # summary(fit_myc_score4) 
...
res1=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=1)
res2=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=2)
res3=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=3)
res4=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=4)
res5=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=5)
res6=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=6)
res7=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=7)
res8=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=8)
res9=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=9)
res10=simu_myc_effect(n=1000,K=10,lambda=1,nb_mean=5,nb_size=1,coef_myc=10)
...
p <- ggplot(final, aes(x=Myc_effect, y=C_index, group = Model, color=Model))+ 
  ...
  labs(title="Simulating Myc's Effect on Patients' Survival",x="Myc-effect multiplier", y = "C-index")+
  theme_classic()+
